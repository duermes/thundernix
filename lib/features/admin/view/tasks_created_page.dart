...
class TasksCreatedPage extends StatelessWidget {
  const TasksCreatedPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Tareas Creadas'),
        actions: [
          PopupMenuButton<String>(
            onSelected: (value) {
              if (value == 'completed') context.go(AppRoutes.adminCompleted);
            },
            itemBuilder: (context) => const [
              PopupMenuItem(value: 'completed', child: Text('Ver Completadas')),
            ],
          )
        ],
      ),
            ...
            children: [
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) => TaskCard(title: tasks[i].$1, subtitle: 'Proyecto', dueLabel: tasks[i].$2, onTap: () {}),
                ),
              )
            ],
          ),
